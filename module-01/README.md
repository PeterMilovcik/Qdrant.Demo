...
A numerical representation (vector) of text that captures its **meaning**. Similar texts produce vectors that are close together in vector space, even if they share no keywords. Think of it as a "GPS coordinate" for meaning.

</details>

<details>
<summary>Why is embedding-based search better than keyword search?</summary>

Keyword search only finds exact word matches. Embedding-based (**semantic**) search matches *meaning* — so "kitten on a rug" matches "cat on a mat" because the vectors are close together, even though the words are completely different.

</details>

<details>
<summary>What does <code>text-embedding-3-small</code> produce and how many dimensions does it have?</summary>

It produces a vector of **1536 floating-point numbers** that represents the semantic meaning of the input text.

</details>

<details>
<summary>What is a "deterministic point ID" and why does it matter?</summary>

A point ID generated by SHA-256 hashing the document's `id` (or its text). The same input always produces the **same UUID**, which makes upserts idempotent — re-indexing overwrites instead of creating duplicates.

</details>

<details>
<summary>What is an idempotent upsert?</summary>

An operation that produces the same result whether you run it once or many times. Upserting a document with the same deterministic ID always overwrites the existing point — so you can safely re-index without creating duplicate vectors.

</details>

<details>
<summary>What does <code>wait: true</code> do when upserting to Qdrant?</summary>

It tells Qdrant to confirm the write is **durable** before returning. This means the point is immediately searchable after the upsert call completes.

</details>

<details>
<summary>How does batch indexing handle partial failures?</summary>

Each document in the batch is processed independently. If one fails (e.g., empty text), the others still succeed. The response reports `succeeded`, `failed`, and an `errors` array with details about what went wrong.

</details>

<details>
<summary>What two payload fields are stored for every indexed document?</summary>

`text` (the original document text) and `indexed_at_ms` (a Unix timestamp in milliseconds recording when the document was indexed).
